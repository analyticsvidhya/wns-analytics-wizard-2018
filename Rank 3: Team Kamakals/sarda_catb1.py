...
catb_tot.reset_index(drop=True)
...
for tr_index,test_index in folds.split(train_catb,y=target):
    ...
    #print f1_maximizer(clf_catb.predict_proba(xval)[:,1],yval)
...
pd.DataFrame({
        '24':( catb_preds>0.2424 ).astype('int'),
        '25':( catb_preds>0.2525 ).astype('int'),
        '26':( catb_preds>0.2626   ).astype('int'),
        '27':( catb_preds>0.2727).astype('int'),
        '28':( catb_preds>0.2828).astype('int')    
            }).to_csv('all_preds_bagged_catb_raw_feats_24_28.csv',index=False)
